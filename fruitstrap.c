...
#define PRINT_INFO(...) if (!quiet) printf(__VA_ARGS__)
...
void usage(const char* app) {
    ...
    printf ("   uninstall  [-i/--id device_id] -b/--bundle bundle id\n");
...
int main(int argc, char *argv[]) {
    ...
    PRINT_INFO( "Welcome! You are using https://github.com/topeerz/fruitstrap dev 0.1 beta\n" );

    char ch;
    while ((ch = getopt_long(argc, argv, "qvt:i:b:da:", global_longopts, NULL)) != -1)
    {
        PRINT_DEBUG( "[debug] detected short param: %c with string %s\n", ch, optarg );
        ...
        case 'i':
            ...
            break;
        default:
            PRINT_ERROR("[error] unknown short param passed: %c", ch);
            usage(argv[0]);
            return 1;
        }
    }
    ...
    if (optind >= argc) {
        PRINT_ERROR("[error] too few parameters passed");
        usage(argv [0]);
        exit (0);
    }

    operation = OP_NONE;
    char* longOption = argv[ optind ];
    if( 0 == strcmp( longOption, "install" ) )
    {
        PRINT_DEBUG("[debug] installation selected");
        operation = OP_INSTALL;
    } else if( 0 == strcmp( longOption, "uninstall" ) ) {
        PRINT_DEBUG("[debug] uninstallation selected");
        operation = OP_UNINSTALL;
    } else if( 0 == strcmp( longOption, "list-devices" ) ) {
        PRINT_DEBUG("[debug] list-devices selected");
        operation = OP_LIST_DEVICES;
    } else {
        PRINT_ERROR("[error] unknown operation selected: %s\n", longOption );
        usage (argv [0]);
        exit (0);
    }

    if (operation != OP_LIST_DEVICES && !app_path) {
        PRINT_ERROR("[error] app_path not specified\n" );
        usage(argv[0]);
        exit(0);
    }
